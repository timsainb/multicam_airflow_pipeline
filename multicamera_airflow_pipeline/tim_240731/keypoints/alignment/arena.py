...
class ArenaAligner:
    ...
    def run(self):
        ...
        if self.plot_steps:
            ...
            axs[1].scatter(
                centroids[::10, 0], centroids[::10, 2], c=outlier_mask[::10], s=1, cmap="coolwarm"
            )
            # save to spikesorting_output_directory
        ...
        z_axis_correlation = scipy.stats.pearsonr(depth_sample, original_depth_sample).statistic
    ...
    def initialize_output(self):
        ...
        coordinates_file = (
            self.arena_alignment_output_directory
            / f"coordinates_aligned.{keypoints_3d_dtype}.{keypoints_3d_shape_str}.mmap"
        )

        centroids_file = (
            self.arena_alignment_output_directory
            / f"centroids.{keypoints_3d_dtype}.{centroids_shape_str}.mmap"
        )
